// lib/features/reports/views/cash_collection_by_agent.dart
...
class CashCollectionByAgent extends StatelessWidget {
  const CashCollectionByAgent({super.key});
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      ...
      child: const _CashCollectionScaffold(),
    );
  }
}

class _CashCollectionScaffold extends StatefulWidget {
  const _CashCollectionScaffold();

  @override
  State<_CashCollectionScaffold> createState() => _CashCollectionScaffoldState();
}

class _CashCollectionScaffoldState extends State<_CashCollectionScaffold> {
  ...
  Future<void> _bootstrap() async {
    ...
    final vm  = context.read<CashCollectionByAgentViewModel>();

    // Wait briefly for product/agent lists to load
    for (int i = 0; i < 20; i++) {
    ...
    if (pvm.filteredItems.isNotEmpty) {
      ...
      // Use 0 as "ALL" agent if backend supports it; else pick first distributorId.
      const agentId = 0;
...
/// ---------------- Filter Dialog (From/To + Product + Agent) ----------------
Future<void> _openFilterDialog(BuildContext context) async {
  ...
  int? selAgentId   = vm.agentId; // 0 => ALL (if supported)
...
class _CashCollectionContent extends StatefulWidget {
  final List<CashReceivedItem> items;
  final bool isLoading;
  final String? error;
  final double totalReceived;

  const _CashCollectionContent({
    required this.items,
  ...
  @override
  State<_CashCollectionContent> createState() => _CashCollectionContentState();
}

class _CashCollectionContentState extends State<_CashCollectionContent> {
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                OutlinedButton(
                  onPressed: () => context.read<CashCollectionByAgentViewModel>().refresh(),
              ...
              Expanded(
                child: Wrap(
                  ...
                  children: [
                    ...
                    Text(
                      'Total Received: ${widget.totalReceived.toStringAsFixed(2)}',
                      style: TextStyle(color: AppTheme.adminWhite.withOpacity(.85), fontSize: 12.sp),
                    ),
                  ],
                ),
        ...
        Expanded(
          child: AnimatedSwitcher(
            ...
            child: _mode == _ViewMode.table
                ? _CashCollectionTable(items: widget.items)
                : _CashCollectionTiles(items: widget.items),
          ),
        ),
      ],
    );
